...
function DownloadPanelImpl({
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    try {
      await downloadAsZip(results, generateZipFilename(), (p) => setZipProgress(p));
      onToast?.(`ZIP downloaded (${results.length} images)`, 'success');
    } catch (error) {
      console.error('Framr: ZIP creation failed', error);
      onToast?.(
        `ZIP failed${error instanceof Error ? `: ${error.message}` : ''}`,
        'error',
      );
    } finally {
      setIsZipping(false);
      ...
      {isProcessing && (
        <div className="space-y-2" aria-live="polite">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-gray-600 dark:text-gray-300" aria-hidden="true">{Math.round(progress)}%</span>
          </div>
          {currentImageName && (
            <p className="text-xs text-gray-600 dark:text-gray-300 truncate">{currentImageName}</p>
          )}
          <div
        ...
        <div className="space-y-3">
          ...
          {isZipping && (
            <div className="space-y-1" aria-live="polite">
              <div className="text-xs text-gray-600 dark:text-gray-300 text-center">Creating ZIP…</div>
              <div
...
export const DownloadPanel = memo(DownloadPanelImpl);
